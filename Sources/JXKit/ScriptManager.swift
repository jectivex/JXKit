...
final class ScriptManager {
    ...
    func evalModule(_ script: String, integratingExports keyPath: String? = nil) throws -> JXValue {
        ...
        if keyPath != nil, context?.configuration.moduleRequireEnabled == true, context?.configuration.isHotReloadEnabled == true, let evalState = evalStack.last {
            return try evalModule(script, resource: nil, integratingExports: keyPath, evalState: evalState)
        } else {
    ...
    private func evalModule(_ script: String?, resource: String?, integratingExports keyPath: String?, evalState: (key: String?, url: URL?, root: URL)) throws -> JXValue {
        ...
        if let script {
            js = script
            if context.configuration.isHotReloadEnabled {
    ...
    private func recordKeyPathReference(_ keyPath: String) {
        guard context?.configuration.isHotReloadEnabled == true, let referencedBy = evalStack.last?.key else {
            return
        }
...
#else // if !canImport(Foundation)
    
    /// Evaluate the given script as a JavaScript module, returning its exports.
    ///
    /// - Parameters:
    ///   - keyPath: If given, the module exports will be integrated into the object at this key path relative to `global`.
    func evalModule(_ script: String, integratingExports keyPath: String? = nil) throws -> JXValue {
        let exports = try evalTransientModule(script)
        try integrate(exports: exports, into: keyPath)
        return exports
